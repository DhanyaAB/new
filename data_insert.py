		...
		if len(l1) == 0:
			...
			for i in last_build_number['builds']:
				...
				if result != 'ABORTED':
					...
					dic1['total statistics']=j[0]
					dic1['statistics by tags']=j[1]
					dic1['statistics by suite']=j[2]
					for i in b:
			...
			for i in l:
				...
				if int(last_build_number) > int(i['output'][0]['buildNumber']):
					...
					st= str(t)[1:-2]
					j=json.loads(st)
					dic2['total statistics']=j[0]
					dic2['statistics by tags']=j[1]
					dic2['statistics by suite']=j[2]
